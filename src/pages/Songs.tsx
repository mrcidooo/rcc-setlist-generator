...
- ＜dyad-write path="src/App.tsx" description="Import Songs correctly and use it in Route">
 Provide snippet.

But the user asked to fix all errors in a concise way. So we can output both fixes.

We must use ＜dyad-write> tags for code.

We also need to include a ＜dyad-chat-summary> at the end with a short summary.

Let's craft the code.

First, Songs.tsx: We'll rewrite it as a clean component, ensuring all braces balanced and export default.

Second, App.tsx: Ensure import and usage.

Let's produce the code.
</think><dyad-write path="src/pages/Songs.tsx" description="Rewrite Songs page as a complete, syntactically correct component with proper exports">
"use client";
...
const mapSong = (record: any): Song => ({
  ...
  addedAt: record.created_at || record.added_at || record.addedAt || "recently",
  notes: record.notes ?? "",
...
export default function Songs() {
  ...
  useEffect(() => {
    setDetectedChords(extractChords(form.lyrics));
  }, [form.lyrics]);

  const availableKeys = useMemo(() => {
    const keys = Array.from(
      new Set(songs.map((s) => (s.originalKey || "").trim())),
    ).sort();
    return ["all", ...keys];
  }, [songs]);
  ...
  const handleAddOrUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!form.title.trim() || !form.originalKey.trim()) {
      toast({ title: "Song not saved", description: "Title and original key are required." });
      return;
    }
    ...
    <div className="min-h-screen bg-transparent p-4 pb-32 max-w-5xl mx-auto space-y-6">
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <CardDescription>
                  Configure song attributes, original keys, lyrics, and chords.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
            ...
            <form onSubmit={handleAddOrUpdate} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  value={form.lyrics}
                  onChange={handleInputChange}
                  placeholder="Example: <Verse> [C] Amazing [G] grace..."
      ...
      <Card className="neu-card border-0 bg-white/75 dark:bg-card/75">
        <CardHeader className="pb-4">
          ...
          <CardDescription>
            Search, filter by original keys, and manage your worship library.
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            <Badge
              variant="secondary"
              className="rounded-full bg-indigo-500/10 text-indigo-500 px-3 py-1 font-bold border-0 text-[10px]"
            >
              {songs.length} total tracks
            </Badge>
            <Badge              variant="outline"
              className="rounded-full px-3 py-1 font-bold text-[10px] border-black/10 dark:border-white/10"
            >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {filteredSongs.map((song) => (
                <SongCard
                  ...
                  className="bg-gradient-to-r from-indigo-900/70 to-purple-900/70 dark:bg-gradient-to-r from-gray-900 to-gray-800 rounded-lg shadow-md p-4 hover:shadow-lg transition-all duration-300"
                />
              ))}
            </div>
          ))}
