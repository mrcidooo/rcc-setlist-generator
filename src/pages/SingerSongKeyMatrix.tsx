...
import { Users, Music, Search, Edit } from "lucide-react";

export default function SingerSongKeyMatrix() {
  // Sample data - in real app this would come from Supabase
  const [songs] = useState<Array<{id: string; title: string; originalKey: string}>>([
  ...
  const [singers] = useState<Array<{id: string; name: string; voiceType: "male" | "female"}>>([
    { id: "1", name: "John Smith", voiceType: "male" },
    { id: "2", name: "Sarah Johnson", voiceType: "female" },
    { id: "3", name: "Mike Davis", voiceType: "male" }
  ]);

  // Sample key matrix - in real app this would be from Supabase
  ...
  const handleKeyChange = (songId: string, singerId: string, newKey: string) => {
    setKeyMatrix(prev => ({
      ...prev,
      [songId]: {
        ...prev[songId],
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4">
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-2">Singer Song Key Matrix</h1>
        <p className="text-gray-600 dark:text-gray-400">
          View and edit the comfortable keys for each singer on each song
        </p>
        <Input
                ...
                {filteredSongs.map((song) => (
                  <tr key={song.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                      {song.title} <span className="text-gray-500 dark:text-gray-400">({song.originalKey})</span>
                    </td>
                    {singers.map((singer) => (
                      <td key={singer.id} className="px-6 py-4 text-sm text-center">
                        <div className="flex items-center justify-center">
                          <span className="mr-2">Key:</span>
                          <Input
                            type="text"
                            value={keyMatrix[song.id]?.[singer.id] || ""}
